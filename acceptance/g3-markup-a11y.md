...
verdict: held
ran: go test ./internal/site -run '^TestA5RequiredPagesAndOperatorLinkGraph$' -count=1 -v
at: 2026-08-28T10:46:39Z
saw: `--- PASS: TestA5RequiredPagesAndOperatorLinkGraph`, exit 0 — the graph test now asserts the legal links are ABSENT from every built page, which is the opposite proposition to the struck criterion
red proof: `grep -rlo 'href="/impressum/"' dist/` and the same for `/datenschutz/` each return 0 files of 31. The criterion cannot be met and is no longer required to be.
notes: The verdict is `held` only because the criterion is struck — ADR 0008, in `main` — so there is no reachability requirement left to fail. The four-word vocabulary has no term for a criterion the contract withdrew, and this is the least-bad of them; read the verdict with this sentence attached. **This row was a false green until 2026-08-28.** Its previous `ran:` line named `TestA5RequiredPagesAndLegalLinkGraph`, which `248dbd4` deleted along with the `a5ReachableWithin` helper. `go test -run` on a pattern matching nothing prints `ok … [no tests to run]` and **exits 0**, so the row re-verified itself by executing zero tests while reproducing its recorded `saw: exit 0` exactly, under `verdict: held`, for a criterion `main` had already struck. Nothing in `tools/acceptance.py` checks whether a `ran:` line still names a live test, so `make acceptance` stayed clean throughout. Predicted by `siduri-reviewer` before the branch that caused it was written, and the first of this wave's findings to reach `main`. The capability is gone rather than inverted: restoring the criterion would not restore its check, because the reachability walk no longer exists.
